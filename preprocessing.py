...
def packet_to_sparse_array(packet, max_length=1500):
    ...
    if len(arr) < max_length:
        ...
        #arr = np.pad(arr, pad_width=(0, pad_width), "constant", constant_values=0)
...
data_path="/media/cs101/DOC1/dataset/pcaps"
for p in os.listdir(data_path)[0]:
    print(p)
    p="ftps_up_2b.pcap"
    #pkt = read_pcap(os.path.join(data_path,p))
    pkt = read_pcap(os.path.join(data_path,p))
    count=0
    #print("pkt",pkt[1:2])
    for pt in pkt:
        count = count+1
        #arr = transform_packet(pt)
        #if count > 3:
        #    count = 0
        #    break
        #print("arr:", arr)
